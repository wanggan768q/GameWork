...
void HSLog::ResponseLog( LogResponse* pMsg )
{
	string path = CCFileUtils::sharedFileUtils()->getWriteablePath() + HS_LOG_FILE_NAME;
	if (pMsg->status() == LogResponse_ResultStatus_OK)
	{
		remove(path.c_str());
	}
	HS_GAME_CACHE()->SetIsSendDeviceInfo(true);
	HS_GAME_CACHE()->Save();
}
...
void HSLog::SendBuyGoodLog(LogRequest_BuyType sendType,int goodId,int goodNumber,LogRequest_BuyState buystate)
{
	...
	m_plastGoodLog->set_buytime(HSTime::GetTime());

	m_plastGoodLog->set_stime(HSTime::GetTime());
...
void HSLog::SendUserLog(LogRequest_ActionType sendType)
{
	...
	m_pLastUserLog->set_stime(HSTime::GetTime());
	...
	if (HSSocketPool::ShareSocketPool()->GetSocketWithTag(DEFAULT_SOCKET)->GetSocketStart() == HS_SOCKET_START_CONNECTED&&m_pLastUserLog->actiontype()!=LogRequest_ActionType_Logout)
	{
		SendLog();
	}
	else
...
void HSLog::SendActivateLog()
{
	...
	m_pLastUserLog->set_logintime(HSTime::GetTime());
}
...
void HSLog::SendLogoutLog()
{
	...
	m_pLastUserLog->set_logouttime(HSTime::GetTime());
}
