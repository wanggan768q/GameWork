...
static HSGameCache* s_hsGameCache = NULL;

HSGameCache::HSGameCache(void)
{
    
    m_lastTargetUUID = "123";
...
HSGameCache::~HSGameCache(void)
{
    ...
    //CC_SAFE_DELETE(m_pTargetInfoResponse);
    CC_SAFE_DELETE(m_pBattleResult);
    CC_SAFE_DELETE(m_pUseBuffRequest);
}
...
void HSGameCache::ReadFigureData()
{
    string fullFilePath = CCFileUtils::sharedFileUtils()->fullPathForFilename("Data/FigureData.data");
    
	HSFileData data(fullFilePath.c_str(),"rb");
    
    FigureData* p = FigureData::default_instance().New();
    
	bool isOk = p->ParsePartialFromArray((const void*)(data.GetBuffer() + 2) ,data.GetSize() - 2);
    ...
    for(int i=0;i<p->figurelist_size();++i)
    {
        FigureData_FigureBase* base = p->mutable_figurelist(i);
        
        m_FigureMap.insert(pair<int,  FigureData_FigureBase*>(base->id(),base));
        
        m_vFigureData.push_back(base);
    }
...
void HSGameCache::Destroy()
{
    ...
    CC_SAFE_DELETE(s_hsGameCache);
}

bool HSGameCache::Save()
{
    string saveFileName = HS_GAME_CACHE_FILE_NAME;
    if (HS_IsLanMode())
    {
        saveFileName = HS_GAME_CACHE_FILE_NAME_LAN;
    }
	std::string path = CCFileUtils::sharedFileUtils()->getWriteablePath() + saveFileName;
...
bool HSGameCache::ReadLan()
{
    std::string path = CCFileUtils::sharedFileUtils()->getWriteablePath() + HS_GAME_CACHE_FILE_NAME_LAN;
...
{
    HS_MUTABLE_TARGET_SELF()->CopyFrom(user);
//	if(user.has_uid())
//	{
...
FigureData_FigureBase* HSGameCache::GetFigureBase(int id)
{
    map<int,FigureData_FigureBase*>::iterator it = m_FigureMap.find(id);
...
FigureData_FigureLevelTable* HSGameCache::GetFigureLevelTable(int id,int level)
{
    FigureData_FigureBase* base = GetFigureBase(id);
    if(base)
    {
        for (int i=0; i<base->leveltablelist_size(); ++i)
        {
            if(base->leveltablelist(i).level() == level)
            {
                return base->mutable_leveltablelist(i);
            }
        }
    }
    return NULL;
...
FigureData_FigureProperty* HSGameCache::GetFigureProperty(int id)
{
    FigureData_FigureBase* base = GetFigureBase(id);
    if(base)
    {
        return base->mutable_property();
    }
    return NULL;
}

vector<FigureData_FigureBase*>* HSGameCache::GetFigureList()
{
    return &m_vFigureData;
}
