...
map<std::string,HSUIData>* HSReadUI::ReadUI(const char* uiDataFile,const char* directory,CCNode* layerNode)
{
    ...
    {
        if (0 == strcmp(uiDataFile, m_allFile[i].c_str()))
        {
    ...
    //return HSReadUI::ReadUI(uiDataFile, NULL, NULL, layerNode);
    ...
    m_allFile.push_back(string(uiDataFile));
	...
	case message::UI_Field::ElementType_Sprite:
		{
			...
			HSAssert(hsSprite,f.resname().c_str());
			...
			if (layer != NULL)
			{
				...
				HSMessageBox("[ %s ] Not components",f.resname().c_str());
				return;
			}			
		}		
		break;
	case message::UI_Field::ElementType_Panel:
		{
			...
			if(f.objectname() != "")
			{
				...
				if(it == pNodeMap->end() )
				{
					...
					HSMessageBox("[ %s ] repeated object name ",f.objectname().c_str());
					return;
				}
			}else{
				HSMessageBox("Panel : Object name cannot be empty! \n x: %2.f    y: %2.f",f.dstpointx(),f.dstpointy());
                return;
			}
...
//    tempFileName.insert(0, "Image/");
    
	string fullFilePath = CCFileUtils::sharedFileUtils()->fullPathForFilename(fileName);
    //string fullFilePath = CCFileUtils::sharedFileUtils()->fullPathForFilename(tempFileName.c_str());
...
void HSReadUI::SetProperty(CCNode* pNode,const message::UI_Field& field)
{
	...
	if(!HSMath::EqualsWithFloat(field.srcpointx(),0.f) && !HSMath::EqualsWithFloat(field.srcpointy(),0.f))
	{
		...
		pos.x = field.dstpointx() + field.imagew() / 2.f;	//œ‡∂‘X
...
//			break;
	...
	if(menuType == UI_MENU_TYPE_NORMAL)
	{
		...
		HSMessageBox("[ %s ]Menu :  This type of no definition,support type 0 or 1 ",menuType.c_str());
	}

	HSAssert(menu != NULL,field.resname().c_str());
...
SPX_Sprite* HSReadUI::HandelSpx(const message::UI_Field& field )
{
	...
	if(spx == NULL)
	{
		HSMessageBox("[ %s ] fail ",spxFileName.c_str());
		return NULL;
	}
