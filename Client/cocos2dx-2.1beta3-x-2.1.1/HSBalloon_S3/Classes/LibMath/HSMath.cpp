...
const char* HSMath::GetUUID(int byteSize /*= 32*/)
{
    string uuid;
    uuid.resize(byteSize);
    
    for (int i=0; i < byteSize; ++i)
    {
        int temp = HSMath::RandomInt();
        
        if (HSMath::RandomInt() % 2)
        {
            uuid[i] = '0' + temp % 10;
            
        }else if(HSMath::RandomInt() % 7){
