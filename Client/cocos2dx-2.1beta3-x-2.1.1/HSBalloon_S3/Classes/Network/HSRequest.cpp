...
void HSRequest::SendUserInfoUpdateRequest_Figure(int FigureID)
{
    HS_GAME_CACHE()->m_UserInfoUpdateRequest_UpdateType = UserInfoUpdateRequest_UpdateType_User_Figure;
    UserInfoUpdateRequest* r = UserInfoUpdateRequest::default_instance().New();
	r->set_type(UserInfoUpdateRequest_UpdateType_User_Figure);
	r->set_figureid(FigureID);
	HS_FILTER_REQUEST(r);
}
...
void HSRequest::SendGameUpgradeFigureLevelRequest()
{
    GameUpgradeFigureLevelRequest* r = GameUpgradeFigureLevelRequest::default_instance().New();
    r->set_id(HS_TARGET_SELF().figureid());
    r->set_level(HS_TARGET_SELF().figurelevel());
    HS_FILTER_REQUEST(r);
    
}

void HSRequest::SendGameSearchStrengthRequest()
{
    GameSearchStrengthRequest* r = GameSearchStrengthRequest::default_instance().New();
    HS_FILTER_REQUEST(r);
}

void HSRequest::SendMatchSearchStrengthRequest()
{
    MatchSearchStrengthRequest* r = MatchSearchStrengthRequest::default_instance().New();
    if(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid() != "")
    {
        r->set_did(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid());
    }
    HS_FILTER_REQUEST(r);
}

void HSRequest::SendGameBuyFigureRequest(int id)
{
    GameBuyFigureRequest* r = GameBuyFigureRequest::default_instance().New();
    r->set_figureid(id);
    HS_FILTER_REQUEST(r);
}

void HSRequest::SendMatchBuyFigureRequest(int figureId)
{
    MatchBuyFigureRequest* r = MatchBuyFigureRequest::default_instance().New();
    if(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid() != "")
    {
        r->set_did(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid());
    }
...
void HSRequest::SendMatchUpgradeFigureLevelRequest()
{
    MatchUpgradeFigureLevelRequest* r = MatchUpgradeFigureLevelRequest::default_instance().New();
    if(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid() != "")
    {
        r->set_did(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid());
    }
...
void HSRequest::SendNotifyUserUpdateRequest()
{
    NotifyUserUpdateRequest* r = NotifyUserUpdateRequest::default_instance().New();
    r->set_did(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid());
    HS_FILTER_REQUEST(r);
}

void HSRequest::SendChangeFigureRequest(int figureId)
{
    ChangeFigureRequest* r = ChangeFigureRequest::default_instance().New();
    r->set_did(HS_GAME_CACHE()->m_pTargetInfoResponse->dungoenid());
    r->set_figureid(figureId);
    HS_FILTER_REQUEST(r);
}

void HSRequest::SendValidateStrengthRequest()
{
    ValidateStrengthRequest* r = ValidateStrengthRequest::default_instance().New();
    HS_FILTER_REQUEST(r);
}
