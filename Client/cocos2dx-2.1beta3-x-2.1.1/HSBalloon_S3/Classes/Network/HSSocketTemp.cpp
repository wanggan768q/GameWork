        ...
        if (code == -1)
        {
            ...
            switch (selectCode)
            {
                ...
                case 0:
                	...
                	CCLog("Connection timeout [code %d]",selectCode);
                    return false;
                case 1:
                {
...
void* HSSocketTemp::Sender(void* arg)
{
	...
	while(pSocketInterface->isLoop)
	{
		...
		if(pSocketInterface->Recv())
		{
			...
			pSocketInterface->m_psendData = NULL;
			HSSocketPool::ShareSocketPool()->ClearPingLongCounter();
		}
		else
		{
