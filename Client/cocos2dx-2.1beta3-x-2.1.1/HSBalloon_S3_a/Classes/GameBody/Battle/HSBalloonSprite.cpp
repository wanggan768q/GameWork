...
void HSBalloonSprite::SetColorType( BalloonColorType type ,float petrifactionTime/* = 0.f*/)
{
	...
	{
		BalloonTexture2D* ballonTexture = &it->second;
		this->setTexture(ballonTexture->base);
		this->SetLuminousTexture2D(ballonTexture->luminous);
	}
...
void HSBalloonSprite::SetLuminousTexture2D( CCTexture2D* pTexture )
{
	m_pLuminousTexture = pTexture;
}
