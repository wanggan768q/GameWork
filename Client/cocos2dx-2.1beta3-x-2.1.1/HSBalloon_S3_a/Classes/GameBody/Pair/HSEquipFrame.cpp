...
bool HSEquipFrame::Init(const message::PropInfo& frameInfo,int index)
{
	...
	this->initWithFile("Image/pair/selectProp/dikuangda.png");
	...
	m_pBell = CCSprite::create("Image/pair/selectProp/Lingdangxiao.png");
...
void HSEquipFrame::Update(float dt)
{
	...
	if(level >= m_pFrameInfo->unlockgold()&&m_lock)//解锁了
	{
		m_lock = false;
...
bool HSEquipFrame::SetPropInfo(const message::PropInfo& propInfo)
{
	...
	m_pPropSprite = CCSprite::create(CCString::createWithFormat("Image/prop/%s",m_pPropInfo->localname().c_str())->getCString());
...
bool HSEquipFrame::ccTouchBegan(CCTouch *pTouch, CCEvent *pEvent)
{
	...
	else
	{
		if(!m_buy)
		{
			...
			if(HS_GAME_CACHE()->GetGold()>this->m_pFrameInfo->unlockgold())
			{
				if(HS_ONLINE)
				{
