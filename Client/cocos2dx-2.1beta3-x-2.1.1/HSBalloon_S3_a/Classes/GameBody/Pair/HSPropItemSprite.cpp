...
bool HSPropItemSprite::Init(const message::PropInfo& propInfo,int index)
{
	...
	const char* fileName = CCString::createWithFormat("Image/prop/%s",m_pPropInfo->localname().c_str())->getCString();
	CCSprite* pNode_0 = CCSprite::create(fileName);
	HSGraySprite* pNode_1 = HSGraySprite::create(fileName);
	CCSprite* pNode_2 = CCSprite::create(fileName);
...
void HSPropItemSprite::Update(float dt)
{
	...
	if(level >= m_pPropInfo->unlocklevel()&&this->lock)//解锁
	{
		this->lock = false;
