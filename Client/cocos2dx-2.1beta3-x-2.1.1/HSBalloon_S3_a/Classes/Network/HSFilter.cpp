...
void HSFilter::CreaterMessage( char* data,int lenth )
{
	...
	for (int i=0;i<infoCount;++i)
	{
		...
		unsigned long long a = HSTime::GetLocalityTime();
		...
		if (commandId != 62010 && commandId != 63010)
		{
			...
			CCLog("\nResponseData(%d %% %d):{\nID:%d -------- %s\n %s \n}\n",i + 1,infoCount,commandId,googleMsg->GetTypeName().c_str(),msgBodyBuf.c_str());
		}
#endif
...
#define HS_REGISTER_PROTOCOL(__Request__,__Response__,__Name__) this->RegisterMessage(__Request__,__Response__,__Name__)
	...
	HS_REGISTER_PROTOCOL(API_FriendInfoRequest,API_FriendInfoResponse,"FriendInfo");
	HS_REGISTER_PROTOCOL(API_DeleteFriendRequest,API_DeleteFriendResponse,"DeleteFriend");
