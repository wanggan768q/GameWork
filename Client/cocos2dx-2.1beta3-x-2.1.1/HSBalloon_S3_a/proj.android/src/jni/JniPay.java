...
import com.letu.iPay.io.LSProtocolConfigs;
import com.letu.iPay.io.LSProtocolKeys;
...

				{
					  ...
					  try 
					  {
						...
						if(code == LSProtocolConfigs.LS_FUNCTION_CODE_INIT)
						{
							...
							{
								 Toast.makeText(HSBalloon_S3_a.getInstance(), "初始化成功", Toast.LENGTH_SHORT).show();
...
//			intent.putExtra(LSProtocolKeys.LS_FUNCTION_GOOD_ID, "2000");
//			intent.putExtra(LSProtocolKeys.LS_FUNCTION_GOOD_SIZE, 1);
				...
				public void onFinish(String data) 
				{
					...
					try {
						...
						if(code == LSProtocolConfigs.LS_FUNCTION_CODE_PAY)
						{
							...
							Toast.makeText(HSBalloon_S3_a.getInstance(), "购买成功 花费RMB="+((float)money/100), Toast.LENGTH_SHORT).show();
