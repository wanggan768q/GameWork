...
void HSGameGuide::Load()
{
	...
	m_pMaskLayer = CCLayerColor::create(ccc4(0,0,0,200),HSBase::GetTemplateScreenW()+10,HSBase::GetTemplateScreenH());
	m_pMaskLayer->setAnchorPoint(HS_ANCHOR_CENTER);
	this->addChild(m_pMaskLayer);
	m_pMaskLayer->setPosition(ccp(-2.f,0));
	HSToolBase::SetNodeFilldScreen(m_pMaskLayer);
	

	m_pDescribeBigFrame = HSCCSprite::create("Image/ShuoMingXian.png");
	m_pDescribeBigFrame->setVisible(false);
	m_pDescribeFrame = HSCCSprite::create("Image/STouMingKuang.png");
	...
	m_pIndicationSprite = HSCCSprite::create("Image/Jiantouyindao.png");
...
void HSGameGuide::Run( int index )
{
	...
	if (m_currentGuide.has_indicationsignpos())
	{
        ...
        m_pIndicationSprite->stopAllActions();
        
        CCPoint indicationPos = m_pIndicationSprite->getPosition();
        CCPoint willPos = ccpSub(indicationPos, ccp(-30.f,-30.f));
        CCMoveTo* pMoveBy_01 = CCMoveTo::create(0.5f, willPos);
        CCMoveTo* pMoveBy_02 = CCMoveTo::create(0.5f, indicationPos);
        CCRepeatForever* pForever = CCRepeatForever::create(CCSequence::create(pMoveBy_01,pMoveBy_02,NULL));
        m_pIndicationSprite->runAction(pForever);
