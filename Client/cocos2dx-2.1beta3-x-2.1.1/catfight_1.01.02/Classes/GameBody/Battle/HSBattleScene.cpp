...
void HSBattleLayer::LoadUI()
{
	...
	if(HS_IsRunGameGuide())
	{
		m_pFinger = HSCCSprite::create("Image/Finger.png");
...
void HSBattleLayer::InitOtherBattleScreen()
{
	...
	if (HS_IsRunGameGuide())
	{
		CCSprite* p = HSCCSprite::create("Image/xinshouyindaotihuan.png");
...
void HSBattleLayer::Logic_Drop( float dt )
{
	...
	if (m_dropCount <= 0)
	{
		m_dropCount = HSMath::Round(1.75f + m_gameTime / 60.f);	//�������ʽ
		tempPassTime = 0.f;
		return;
	}
...
void HSBattleLayer::ResponseBattleResult( BattleResultResponse* pMsg )
{
	...
	if(pMsg->result() == BattleResultResponse_PlayerMatchResult_WIN)//���Ӯ�ˣ�Ҫ�Ȳ���Ч��
	{
...
void HSBattleLayer::PlayUserPropEffect_OtherScreen(const PropInfo* propInfo)
{
	...
	CCString* pIconFileName = CCString::createWithFormat("Image/%s",propInfo->localname().c_str());
...
void HSBattleLayer::Logic_GameGuide( float dt )
{
	...
	if (s_isRun_1013 && currentStepID == 1013)
	{
		...
		CCSprite* s = HSCCSprite::create("Image/animal_1.png");
