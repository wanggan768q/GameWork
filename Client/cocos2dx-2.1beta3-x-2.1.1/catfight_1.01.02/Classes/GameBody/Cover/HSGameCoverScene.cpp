...
{
	HSReadUI::ShareReadUI()->ReadUI("Image/main.data","Image/",this);
    ...
    HS_SET_MENU_TARGET("UI_HEbinggaojiduchangxin_Haoyou_01",this,HSGameCoverLayer::CallFriend);
    ...
    CCTexture2D* pMoneyNumberTexture = CCTextureCache::sharedTextureCache()->addImage("Image/moneyNumber.png");
	m_pGameGold = CCLabelAtlas::create("0","Image/moneyNumber.png",pMoneyNumberTexture->getPixelsWide() / 11,pMoneyNumberTexture->getPixelsHigh(),'0');
    ...
    CCNode* pGouMai = HS_FIND_UI_MENU("UI_HEbinggaojiduchangxin_Goumaijinbianniu_01");
    pGouMai->setVisible(false);
}

void HSGameCoverLayer::CallChange(CCObject* obj)
{
	if(!HSGamePlayerInfoLayer::SharedPlayerInfoLayer()->isVisible())
	{
		if(!m_changing)
			{
				m_changing = true;
				...
				if(m_CurIndex == 0)
				{
					...
					dict["type"] = "senior";
					MobClickCpp::event("seniorButtonClick",&dict);
...
bool HSGameCoverLayer::RequestFighting()
{
	if(HS_ONLINE)
	{
        //普通场
        if(HSGamePairScene::s_areaType==area_normal)
        {
            if(HS_GAME_CACHE()->GetGold()<=0)//跳转计费sdk 充值默认最低的钱数
            {
                HSJava::ShaderJava()->Pay(1);
                return false;
            }
            else
            {
                HS_GAME_CACHE()->roomGold = 1;
                HS_GAME_CACHE()->m_roomId = 0;
...
void HSGameCoverLayer::ResponseTargetInfo(TargetInfoResponse* responseMsg)
{
	...
	CCLog("target uid=%s",responseMsg->target().uid().c_str());
...
void HSGameCoverLayer::ResponseFriendList( FriendListResponse* r )
{
...
{
	HSReadUI::ShareReadUI()->ReadUI("Image/normalback.data","Image/",this);
    
//    CCNode* pweibo = HS_FIND_UI_PANEL("UI_Dengluanniu");
//    pweibo->setVisible(false);
//
//    CCNode* fastlogin = HS_FIND_UI_PANEL("UI_Kuaisudenglu");
...
{
	m_p_normal_backGround = CCSprite::create("Image/main_backGround.png");
...
{
	m_p_senior_foreground = HSCCSprite::create("Image/seniorforeground.png");
    
	m_p_senior_backGround = HSCCSprite::create("Image/seniorbackground.png");
    
	m_p_senior_catblack1 = HSCCSprite::create("Image/main_catblack1.png");
    
	m_p_senior_catblack2 = HSCCSprite::create("Image/main_catblack2.png");
