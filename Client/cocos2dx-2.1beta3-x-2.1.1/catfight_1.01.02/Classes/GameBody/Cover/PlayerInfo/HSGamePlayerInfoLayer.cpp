...
{
	m_p_background = HSCCSprite::create("Image/BEIJING.png");

	//m_p_background->setPosition(HSBase::GetScreenCentre());
	m_p_background->setPosition(ccp(HSBase::GetScreenWidth() / 2.f-1.f,HSBase::GetScreenHeight() / 2.f));
	...
	HSReadUI::ShareReadUI()->ReadUI("Image/playerInfo.data","Image/",m_p_foreLayer);
	...
	CCSprite* pheadFrame = HS_FIND_UI_PANEL_SPRITE("UI_TOUXIANG","Touxiangdikuang");
	const char* pheadPath = CCString::createWithFormat("Image/%d.png",HS_TARGET_SELF().iconid())->getCString();
	m_pHead = HSCCSprite::create(pheadPath);
...
void HSGamePlayerInfoLayer::CloseChangeHead(CCNode* pTarget,void* data)
{
	...
	if(HS_GAME_CACHE()->GetGold()>=HS_GAME_CACHE()->GetPropInfoData()->headlist(index).gold())
	{

		HS_GAME_CACHE()->changeheadIndex = index;

		HSRequest::ShareRequest()->SendUserInfoUpdateRequest_IconId(HS_GAME_CACHE()->GetPropInfoData()->headlist(index).id());
	}
	else//跳转sdk计费
	{
		HSJava::ShaderJava()->Pay(HS_GAME_CACHE()->GetPropInfoData()->headlist(index).gold()-HS_GAME_CACHE()->GetGold());
	}
...
void HSGamePlayerInfoLayer::Update(float dt)
{
	if(m_iconId !=HS_TARGET_SELF().iconid())
	{
		...
		const char* pheadPath = CCString::createWithFormat("Image/%d.png",m_iconId)->getCString();
