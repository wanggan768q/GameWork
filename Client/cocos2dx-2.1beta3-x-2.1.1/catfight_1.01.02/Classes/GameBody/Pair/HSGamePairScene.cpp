...
void HSGamePairLayer::Load()
{
	...
	HSReadUI::ShareReadUI()->ReadUI("Image/normalPair.data","Image/",this);
	...
	m_pSelfLevelTTF = CCLabelTTF::create(CCString::createWithFormat("Lv.%d",HS_TARGET_SELF().level())->getCString(),HS_FONT_HuaKang,36.f,pLevelFrame->getContentSize(),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter);
    ...
    CCSpriteFrame* pNewExperienceFrame = HS_RP_GetSpriteFrame("Image/jingyantiaozhuose.png");
	HSCCSprite* pExperienceSprite = HSCCSprite::create(pNewExperienceFrame);
	...
	m_pGameReadTime = CCLabelAtlas::create(CCString::createWithFormat("%d",m_readyTime)->getCString(),"Image/moneyNumber.png",pTexture->getPixelsWide() / 11,pTexture->getPixelsHigh(),'0');
...
void HSGamePairLayer::VSShade()
{
	...
	CCSprite* vs = HSCCSprite::create("Image/Vsduizhan.png");
...
void HSGamePairLayer::ShowHead()
{
	...
	const char* selfPath = CCString::createWithFormat("Image/%d.png",HS_TARGET_SELF().iconid())->getCString();

	CCSprite* headSelf = HSCCSprite::create(selfPath);
	...
	m_pSelfMark = HSCCSprite::create("Image/mask_Nianyeanniu.png");
	...
	const char* EnemyPath = CCString::createWithFormat("Image/%d.png",HS_TARGET_TARGET().iconid())->getCString();
	CCSprite* headEnemy = HSCCSprite::create(EnemyPath);
	...
	m_pTargetMark = HSCCSprite::create("Image/mask_Nianyeanniu.png");
	...
	m_pBufferInfoTTF = CCLabelTTF::create("",HS_FONT_HuaKang,36.f,pBufferInfoFrame->getContentSize(),kCCTextAlignmentCenter,kCCVerticalTextAlignmentTop);
...
void HSGamePairLayer::CallReturn(CCObject* obj)
{
	if (HS_IsRunGameGuide())
	{
        ...
        if (HS_GAME_CACHE_DATA()->isalreadyrungameguide())
        {
            ...
            HS_MUTABLE_GAME_CACHE_DATA()->set_isalreadyrungameguide(true);
            HS_GAME_CACHE()->Save();
...
void HSGamePairLayer::UpdateEffect()
{
	if(HS_TARGET_SELF().effect().refrash())
	{
		...
		else
		{
			...
			CCSpriteFrame* pNewTexture= NULL;
			...
			case message::PropInfo_EffectPropType_Effect_SickMucus:
                {
                    HSEffect_SickMucusLayer::create();
                    CCString* pMaskStr = CCString::createWithFormat("Image/mask_%s",pProp->localname().c_str());
                    pNewTexture = HS_RP_GetSpriteFrame(pMaskStr->getCString());
                    m_pSelfMark->setDisplayFrame(pNewTexture);
			...
			m_pBufferInfoTTF->setString(pBufferInfo->getCString());
			...
			case message::PropInfo_EffectPropType_Effect_SickMucus:
                {
                    ...
                    CCString* pMaskStr = CCString::createWithFormat("Image/mask_%s",pProp->localname().c_str());
                    pNewTexture = HS_RP_GetSpriteFrame(pMaskStr->getCString());
                    m_pTargetMark->setDisplayFrame(pNewTexture);
...
//			CCSprite* pBufferInfoFrame = HS_FIND_UI_PANEL_SPRITE("UI_hengfuwenbenkuang","Shuomingdaditu");
			...
			//CCLog("target---------------------------------%s",HS_TARGET_TARGET().name().c_str());
			CCString* pBufferInfo = CCString::createWithFormat(L("UseBufferInfo1"),HS_TARGET_TARGET().name().c_str(),bufferName.c_str());
			const char* c = pBufferInfo->getCString();

			m_pBufferInfoTTF->setString(c);
//			CCLog("target ---- %s",pBufferInfo->getCString());
//			m_pBufferInfoTTF = CCLabelTTF::create(pBufferInfo->getCString(),HS_FONT_HuaKang,36.f,pBufferInfoFrame->getContentSize(),kCCTextAlignmentCenter,kCCVerticalTextAlignmentTop);
			m_pBufferInfoTTF->setAnchorPoint(HS_ANCHOR_CENTER);
			m_pBufferInfoTTF->setPosition(m_pBufferInfoTTF->getAnchorPointInPoints());
//			pBufferInfoFrame->addChild(m_pBufferInfoTTF);
...
{
	if(r->status() != TargetInfoResponse_FindStatus_NOMONEY)//如果因为没钱了，这里不做处理，等待服务器踢出
	{
...
void HSGamePairLayer::Call_ListerTarget()
{
	...
	if (HS_MUTABLE_TARGET_TARGET()->uid().length() != 32)
	{
