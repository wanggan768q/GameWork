...
//		output.write("					  $(LOCAL_PATH)/../../../extensions/network" + " \\\n");
		...
		output.write("LOCAL_WHOLE_STATIC_LIBRARIES := jsoncpp_static cocos2dx_static cocosdenshion_static cocos_extension_static" + " \n");
	...
	public void init() {
		...
		System.out.println(properties.getProperty("mkFilePath"));
		System.out.println("项目:HSBalloon_S3_a");
		System.out.println("******************************************************");

		File fileRead = new File(sourceFileName);
		File[] fileList = fileRead.listFiles();

		File file = new File(properties.getProperty("mkFilePath"));
		if (file.exists()) {
			file.delete();
		}
		// try {
