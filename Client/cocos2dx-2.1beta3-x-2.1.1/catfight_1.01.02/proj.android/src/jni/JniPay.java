				...
				public void onFinish(String data) 
				{
					...
					try 
					{
						...
						if(code == LSProtocolConfigs.LS_FUNCTION_CODE_PAY)
						{
							...
							float fmoney = (float)money/100;
							
							if(status == 0)//计费成功
							{
								Toast.makeText(HSBalloon_S3_a.getInstance(), "购买成功 花费人民币"+fmoney+"元", Toast.LENGTH_SHORT).show();
							}
						...
						{
							Toast.makeText(HSBalloon_S3_a.getInstance(), "购买失败 返回code错误，code="+code+",应该是"+LSProtocolConfigs.LS_FUNCTION_CODE_PAY, Toast.LENGTH_SHORT).show();
							finishPay(LSProtocolConfigs.LS_FUNCTION_STATUS_ERROR,"",0,"");
						}
					} 
					catch (JSONException e) 
					{
						 Toast.makeText(HSBalloon_S3_a.getInstance(), "购买失败，sdk返回数据解析错误", Toast.LENGTH_SHORT).show();
						 e.printStackTrace();
					}
				}
			});
